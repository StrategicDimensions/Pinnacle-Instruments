...
class ReportStatementCommon(models.AbstractModel):
    ...
    def _show_buckets_sql_q1(self, partners, date_end, account_type):
        return str(
            ...
            END AS open_due,
            CASE WHEN l.balance > 0.0
                THEN l.amount_currency - sum(coalesce(pd.credit_amount_currency, 0.0))
    ...
    def _get_report_values(self, docids, data=None):
        ...
        for partner_id in partner_ids:
            ...
            for line in balances_forward.get(partner_id, []):
                zar_currency_id = self.env["res.currency"].search([('name', '=', 'ZAR')])
                if zar_currency_id.id == line["currency_id"]:
                    (
                        currency_dict[line["currency_id"]],
                        currencies,
                    ) = self._get_line_currency_defaults(
                        line["currency_id"], currencies, total_balance + line["balance"]
                    )
                    total_balance = total_balance +line["balance"]
                else:
                    (
                        currency_dict[line["currency_id"]],
                        currencies,
